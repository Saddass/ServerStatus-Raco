...
exports.start = function(SETUP) {
  ...
  const SUGGESTION_CHANNEL = "847983476312768512"
  const BUG_CHANNEL = "847983476312768512"
  const BUG_LOG_CHANNEL = "847983476312768512"
  const LOG_CHANNEL = "847983476312768512"
  const STREAM_URL = SETUP.STREAM_URL;
  const STREAM_CHANNEL = SETUP.STREAM_CHANNEL;
  const UPDATE_TIME = 2500; // in ms

  const ServerName = SETUP.ServerName;
  const LogoURL = SETUP.LogoURL;
  const TeamspeakIP = SETUP.TeamspeakIP;
  const ConnectIP = SETUP.ConnectIP;
  const BannerURL = SETUP.BannerURL;
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor(`${ServerName} Server Status`, `${LogoURL}`)
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? `⚪ ${ServerName} 𝐒𝐞𝐫𝐯𝐞𝐫` : `⚫ ${ServerName} 𝐒𝐞𝐫𝐯𝐞𝐫`)
    .setTimestamp(new Date())
    .addField('\` ملاحظة مهمة قبل الدخول الى السيرفر: \` { `🟡` }','\`\`\`Steam يجب عليك التاكد من تشغيل \`\`\`',false)
    .addField('\`الدخول السريع من "F8" \` { `🟠` }',`\`\`\`connect ${ConnectIP}\`\`\``,false)
    .addField('\`التيم سبيك \` { `🔵` }',`\`\`\`${TeamspeakIP}\`\`\``,false)
    .addField('ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ','\n\u200b\n',false)
    .setImage(`${BannerURL}`)
    /*.setAuthor("ALRP Server Status", "https://cdn.discordapp.com/attachments/848759928822956092/855194595701030952/ArabLeaders_logo_no_background.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ 𝐀𝐋𝐑𝐏 𝐒𝐞𝐫𝐯𝐞𝐫' : '⚫ 𝐀𝐋𝐑𝐏 𝐒𝐞𝐫𝐯𝐞𝐫')
    .setTimestamp(new Date())
    .addField('\`حالة السيرفر \` { \`⚪\` }','\`\`\`Soon\`\`\`',false)
    .setImage(`https://cdn.discordapp.com/attachments/848759928822956092/855194595701030952/ArabLeaders_logo_no_background.png`)*/
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('\`حالة السيرفر :\` { `🔴` }','\`\`\`Offline / غير متصل\`\`\`',false)
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('\`حالة السيرفر :\` { `🟢` }','\`\`\`Online / متصل\`\`\`',false)
        //.addField('Wachtrij',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        //.addField('\`عدد اللاعبين :\` { 🎮 }',`**{ ${players.length}/${MAX_PLAYERS} }**\n\u200b\n`,false)
        .addField('\`عدد اللاعبين :\` { `🎮` }',`**{ ${players.length}/${MAX_PLAYERS} }**`,false)
       .addField('\u200b','**ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ**\u200b',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
          ...
          for (var i=0;i<fields.length;i++) {
            let field = fields[i];
            if (field.length > 0) embed.addField('--',field,false);
            //},field,false);
        }
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('𝐀𝐋𝐑𝐏 𝐁𝐨𝐭',{'url':'https://www.twitch.tv/zraco_', 'type':'STREAMING'});
